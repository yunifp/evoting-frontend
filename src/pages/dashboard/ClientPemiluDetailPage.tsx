...
import { Vote, Users, Plus, Trash2, ArrowLeft, UserPlus, Info, Calendar, X, Image as ImageIcon, Clock } from 'lucide-react';

export default function ClientPemiluDetailPage() {
    const { id } = useParams();
    const navigate = useNavigate();
const { pemilu, dpts, isLoading, fetchDetail, fetchDpts, addKandidat, deleteKandidat, addDpt, publishPemilu } = usePemiluDetail(id!);
    // Modal States
    ...
    const [dptForm, setDptForm] = useState({ nik: '', nama: '', no_hp: '' });
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 max-w-7xl mx-auto pb-12">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm relative overflow-hidden">
                ...
                <div className="relative z-10 flex shrink-0">
                    {pemilu.status === 'draft' ? (
                        <Button
                            onClick={() => {
                                if (window.confirm("Apakah Anda yakin ingin mengaktifkan acara ini? Setelah aktif, pemilih terdaftar dapat login dan melakukan voting.")) {
                                    publishPemilu(); // Fungsi ini sudah ada di hook Anda
                                }
                            }}
                        ...
                        <Badge className={`px-5 py-2.5 rounded-2xl uppercase tracking-widest text-sm font-black shadow-sm ${pemilu.status === 'active' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-gray-100 text-gray-700 border border-gray-200'
                            }`}>
            ...
            {/* PERBAIKAN TABS DI SINI: Menggunakan flex-col pada wrapper dan flex-row pada list agar rapi */}
                        ...
                        <div className="p-6 md:p-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
                            <form id="kandidat-form" onSubmit={handleAddKandidat} className="space-y-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <p className="text-xs font-medium text-gray-500">Masukkan link gambar (opsional). Biarkan kosong jika belum ada.</p>
                                </div>
                        ...
                        <div className="p-6 md:p-8">
                            <form id="dpt-form" onSubmit={handleAddDpt} className="space-y-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    />
                                    <p className="text-xs font-medium text-amber-600 bg-amber-50 p-2 rounded-lg mt-2 border border-amber-100">Penting: NIK ini akan digunakan oleh pemilih sebagai Username saat Login.</p>
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <p className="text-xs font-medium text-gray-500">OTP keamanan akan dikirimkan ke nomor ini (jika fitur aktif).</p>
                                </div>
