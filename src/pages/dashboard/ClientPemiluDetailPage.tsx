...
import { Vote, Users, Plus, Trash2, ArrowLeft, UserPlus, Info, Calendar, X, Image as ImageIcon, Clock, ScanFace, MapPin, Fingerprint } from 'lucide-react';
...
export default function ClientPemiluDetailPage() {
    ...
    const [dptForm, setDptForm] = useState(initialDptForm);
    ...
    }, [isDptModalOpen, getProvinsi, getStatusKawin]);
    ...
    const handleAddDpt = async (e: React.FormEvent) => {
        ...
        setIsSubmitting(true);
        try {
            const result = await addDpt(dptForm);
            setIsDptModalOpen(false);
            setDptForm(initialDptForm);
            fetchDpts();
                            ...
                            <Table className="min-w-[800px]">
                                <TableHeader className="bg-gray-50/80">
                                    <TableRow className="border-b border-gray-100 hover:bg-transparent">
                                        <TableHead className="font-extrabold text-gray-900 py-5 px-6">ID / UUID</TableHead>
                                        <TableHead className="font-extrabold text-gray-900">Identitas (NIK / NKK)</TableHead>
                                ...
                                <TableBody>
                                    {dpts.map((d) => (
                                        <TableRow key={d.id} className="hover:bg-gray-50/50 border-b border-gray-50 transition-colors group">
                                            ...
                                            <TableCell>
                                                <div className="flex flex-col">
                                                    <span className="font-black text-gray-900 text-base">{d.nama}</span>
                                                    {d.jenis_kelamin && <span className="text-xs text-gray-500">{d.jenis_kelamin}</span>}
                                                </div>
                                            </TableCell>
                        ...
                        <div className="p-6 md:p-8 overflow-y-auto custom-scrollbar flex-1">
                            <form id="kandidat-form" onSubmit={handleAddKandidat} className="space-y-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <textarea value={kandidatForm.visi} onChange={(e) => setKandidatForm({ ...kandidatForm, visi: e.target.value })} className="w-full rounded-2xl border border-gray-200 bg-gray-50 p-4 text-base font-medium focus:ring-4 focus:ring-[#12b3d6]/10 focus:border-[#12b3d6] transition-all min-h-[100px]" placeholder="Tuliskan visi kandidat di sini..." />
                                </div>
                                <div className="space-y-2">
                                    <Label className="text-gray-700 font-bold text-sm uppercase tracking-wide">Misi</Label>
                                    <textarea value={kandidatForm.misi} onChange={(e) => setKandidatForm({ ...kandidatForm, misi: e.target.value })} className="w-full rounded-2xl border border-gray-200 bg-gray-50 p-4 text-base font-medium focus:ring-4 focus:ring-[#12b3d6]/10 focus:border-[#12b3d6] transition-all min-h-[140px]" placeholder="1. Misi pertama&#10;2. Misi kedua..." />
                                </div>
                            </form>
                        </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
                    <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-4xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col max-h-[95vh]">
                        ...
                        <div className="p-6 md:p-8 overflow-y-auto custom-scrollbar flex-1 bg-white">
                            <form id="dpt-form" onSubmit={handleAddDpt} className="space-y-8">
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                        <div className="space-y-2">
                                            ...
                                            <Input required minLength={16} maxLength={16} value={dptForm.nik} onChange={(e) => setDptForm({ ...dptForm, nik: e.target.value.replace(/\D/g, '') })} className="h-12 rounded-xl font-mono text-sm tracking-widest bg-gray-50" placeholder="16 Digit NIK" />
                                        </div>
                                        <div className="space-y-2">
                                            <Label className="text-gray-700 font-bold text-xs uppercase tracking-wide">No. KK (NKK)</Label>
                                            <Input maxLength={16} value={dptForm.nkk} onChange={(e) => setDptForm({ ...dptForm, nkk: e.target.value.replace(/\D/g, '') })} className="h-12 rounded-xl font-mono text-sm tracking-widest bg-gray-50" placeholder="16 Digit NKK" />
                                        </div>
                                        <div className="space-y-2">
                                            ...
                                            <Input required value={dptForm.nama} onChange={(e) => setDptForm({ ...dptForm, nama: e.target.value })} className="h-12 rounded-xl font-bold bg-gray-50" placeholder="Mochammad Fulan" />
                                        </div>
                                        <div className="space-y-2">
                                            <Label className="text-gray-700 font-bold text-xs uppercase tracking-wide">Nama Penduduk (Alias/Panggilan)</Label>
                                            <Input value={dptForm.nama_penduduk} onChange={(e) => setDptForm({ ...dptForm, nama_penduduk: e.target.value })} className="h-12 rounded-xl bg-gray-50" placeholder="Fulan" />
                                        </div>
                                        <div className="space-y-2">
                                            <Label className="text-gray-700 font-bold text-xs uppercase tracking-wide">Tempat Lahir</Label>
                                            <Input value={dptForm.tempat_lahir} onChange={(e) => setDptForm({ ...dptForm, tempat_lahir: e.target.value })} className="h-12 rounded-xl bg-gray-50" placeholder="Jakarta" />
                                        </div>
                                        <div className="space-y-2">
                                            <Label className="text-gray-700 font-bold text-xs uppercase tracking-wide">Jenis Kelamin</Label>
                                            <select value={dptForm.jenis_kelamin} onChange={(e) => setDptForm({ ...dptForm, jenis_kelamin: e.target.value })} className="w-full h-12 rounded-xl border border-gray-200 bg-gray-50 px-3 outline-none text-sm font-medium">
                                        ...
                                        <div className="space-y-2 md:col-span-2">
                                            ...
                                            <Input required maxLength={15} value={dptForm.no_hp} onChange={(e) => setDptForm({ ...dptForm, no_hp: e.target.value.replace(/\D/g, '') })} className="h-12 rounded-xl font-bold bg-gray-50" placeholder="08123456789" />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
                                        <div className="space-y-2 md:col-span-4">
                                            ...
                                            <Input value={dptForm.alamat} onChange={(e) => setDptForm({ ...dptForm, alamat: e.target.value })} className="h-12 rounded-xl bg-gray-50" placeholder="Jl. Merdeka No. 10" />
                                        </div>
                                        <div className="space-y-2 md:col-span-2">
                                            <Label className="text-gray-700 font-bold text-xs uppercase tracking-wide">RT</Label>
                                            <Input maxLength={5} value={dptForm.rt} onChange={(e) => setDptForm({ ...dptForm, rt: e.target.value })} className="h-12 rounded-xl bg-gray-50" placeholder="001" />
                                        </div>
                                        <div className="space-y-2 md:col-span-2">
                                            <Label className="text-gray-700 font-bold text-xs uppercase tracking-wide">RW</Label>
                                            <Input maxLength={5} value={dptForm.rw} onChange={(e) => setDptForm({ ...dptForm, rw: e.target.value })} className="h-12 rounded-xl bg-gray-50" placeholder="002" />
                                        </div>
                                        
                                        <div className="space-y-2 md:col-span-1">
                                            <Label className="text-gray-700 font-bold text-xs uppercase tracking-wide">Provinsi</Label>
                                            <select
                                                value={dptForm.kode_pro}
                                                onChange={(e) => {
                                                    const selected = provinsi.find((p: any) => p.id === e.target.value);
                                                    setDptForm({
                                                        ...dptForm,
                                                        kode_pro: selected?.id || '',
                                                        nama_pro: selected?.name || '',
                                                        kode_kab: '', nama_kab: '',
                                                        kode_kec: '', nama_kec: '',
                                                        kode_kel: '', nama_desa: ''
                                                    });
                                        ...
                                        <div className="space-y-2 md:col-span-1">
                                            ...
                                            <select
                                                disabled={!dptForm.kode_pro}
                                                value={dptForm.kode_kab}
                                                onChange={(e) => {
                                                    const selected = kabupaten.find((p: any) => p.id === e.target.value);
                                                    setDptForm({
                                                        ...dptForm,
                                                        kode_kab: selected?.id || '',
                                                        nama_kab: selected?.name || '',
                                                        kode_kec: '', nama_kec: '',
                                                        kode_kel: '', nama_desa: ''
                                                    });
                                        ...
                                        <div className="space-y-2 md:col-span-1">
                                            ...
                                            <select
                                                disabled={!dptForm.kode_kab}
                                                value={dptForm.kode_kec}
                                                onChange={(e) => {
                                                    const selected = kecamatan.find((p: any) => p.id === e.target.value);
                                                    setDptForm({
                                                        ...dptForm,
                                                        kode_kec: selected?.id || '',
                                                        nama_kec: selected?.name || '',
                                                        kode_kel: '', nama_desa: ''
                                                    });
                                        ...
                                        <div className="space-y-2 md:col-span-1">
                                            ...
                                            <select
                                                disabled={!dptForm.kode_kec}
                                                value={dptForm.kode_kel}
                                                onChange={(e) => {
                                                    const selected = kelurahan.find((p: any) => p.id === e.target.value);
                                                    setDptForm({
                                                        ...dptForm,
                                                        kode_kel: selected?.id || '',
                                                        nama_desa: selected?.name || ''
                                                    });
                                                }}
                                                className="w-full h-12 rounded-xl border border-gray-200 bg-gray-50 px-3 outline-none text-sm font-medium disabled:opacity-50"
                                            >
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                        <div className="space-y-2">
                                            ...
                                            <select 
                                                value={dptForm.status_kawin} 
                                                onChange={(e) => setDptForm({ ...dptForm, status_kawin: e.target.value })} 
                                                className="w-full h-12 rounded-xl border border-gray-200 bg-gray-50 px-3 outline-none text-sm font-medium"
                                            >
                                                <option value="">Pilih...</option>
                                                {statusKawin.map((s: any) => (
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Input value={dptForm.status_disabilitas} onChange={(e) => setDptForm({ ...dptForm, status_disabilitas: e.target.value })} className="h-12 rounded-xl bg-gray-50" placeholder="Normal / Jenis Disabilitas" />
                                        </div>
